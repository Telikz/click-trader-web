 ...
 *
 * Obtain a handle from the [`stock`] property on [`RemoteTables`],
 * like `ctx.db.stock`.
 *
 ...
 */
export class StockTableHandle {
  tableCache: TableCache<Stock>;

  constructor(tableCache: TableCache<Stock>) {
  ...
  /**
   * Access to the `id` unique index on the table `stock`,
   * which allows point queries on the field of the same name
   * via the [`StockIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.stock.id().find(...)`.
   *
   * Get a handle on the `id` unique index on the table `stock`.
   */
  id = {
